...
<div class="container d-flex justify-content-center" style="margin-bottom: 20px; margin-top: 20px">
            <div class="form-row align-items-center fundo col-auto">

   <form method="POST" action="envia.php" enctype="multipart/form-data">
   <div class="col-auto">
                          <?php
    include "conexao.php";
    $iffar = "SELECT * FROM iffar WHERE id_iffar = 1";
    $query = mysqli_query($conectar, $iffar);
    $informacoes = mysqli_fetch_assoc($query);

    echo '<b>'."O que é o IFFAR - FW: ".'</b>' . '<p>'.$informacoes['oque'].'</p>';
    echo '<hr>';
    echo '<b>'."Sobre o IFFAR - FW: ".'</b>' . '<p>'.$informacoes['sobre'];   
    echo '<hr>';
    echo '<b>'."Onde fica o IFFAR - FW: ".'</b>'. '<p>'.$informacoes['local_iffar'];
    echo '<hr>';
    echo '<b>'."Onde fica o IFFAR - FW: ".'</b>'. '<p>'. $informacoes['contato'];
